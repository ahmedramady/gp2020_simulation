...
class gui(QDialog):
    ...
    def __init__(self):
        ...
        loadUi('/home/mariemadel/catkin_ws/src/gp2020_simulation/gp2020_simulation/gui/gui.ui', self)
    ...
    def switchLane(self):
        ...
        self.lane_pub.publish(currentValue + 1)
	...
	#if self.currentStatus == "safe" or self.currentStatus == "slow":
    ...
    def checkStatus (self):
        ...
        if self.currentStatus == "safe":
	    print("current status: no object in distance.. moving forward")
        elif self.currentStatus == "slow":
            self.speedLevel(130)
	    print("current status: warning.. decreasing speed")
        else:
            self.stopbutton.click()
	    print("current status: danger!!stopping..")
    ...
    def laserData(self, msg):
        if(msg.ranges[360] < 0.7 and msg.ranges[360] > 0.45):
            ...
            #self.currentStatus = "slow"
        elif(msg.ranges[360] >=1):
            self.depthSafeSignal.emit(msg.ranges[360])
            #self.currentStatus = "safe"
    ...
    def lane_callback(self,lane_msg):
		...
		if(lane_msg.data == 101):
		    ...
		    self.maxSteer = abs(float(0.35*self.current_slope))
		    self.rightbutton.click()
		    print("turning right with angle", self.maxSteer)
		elif (lane_msg.data == -110):
            	    self.maxSteer = abs(float(0.35*self.current_slope))
		    self.leftbutton.click()
		    #self.forwardbutton.click()
		    print("turning left with angle", self.maxSteer)
		else:
		    self.stopbutton.click()
